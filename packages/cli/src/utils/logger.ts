...
export const logger = {
  ...
  banner() {
    console.log();
    console.log(
      "  " + pc.bold(pc.cyan("@ss/next-jwt-auth")) + pc.gray("  v0.1.0"),
    );
    console.log(
  ...
  nextSteps(authDir: string, hasMiddleware: boolean) {
    ...
    console.log("     " + pc.cyan(`import { auth } from "@/auth"`));
    console.log(
      "     " +
        pc.gray(
          "auth.getSession()  ·  auth.requireSession()  ·  auth.getUser()",
        ),
    );
    console.log();
    if (hasMiddleware) {
    ...
    console.log(
      "  " + pc.gray("Library installed at: ") + pc.cyan(authDir + "/"),
    );
    console.log();
  },
};
